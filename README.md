...
OpenVisualizer (OV) is part of UC Berkeley's OpenWSN project. It provides monitoring, visualization and simulation of OpenWSN-based wireless sensor network. See the project [home page](https://openwsn.atlassian.net/wiki/display/OW/OpenVisualizer) for more information. The project works in tandem with the OpenWSN firmware hosted at [openwsn-fw](https://github.com/openwsn-berkeley/openwsn-fw). OpenVisualizer interfaces with locally connected hardware, the OpenTestBed infrastructure, [IoT-LAB](https://www.iot-lab.info/) or an emulated OpenWSN network.
...
Once installed you need to activate the virtual environment. <br>
(If you want to run OpenVisualizer in a [Docker container](#docker) , you can skip this step. But from this point on, we assume that you have one kind of virtual environment set up.) <br>
On Linux or OSX:
...
* Connect to [IoT-LAB](https://www.iot-lab.info/), see [iotlab](#iotlab)
...
```bash
(venv) $ openv-server --sim <N>
```
with N being the number of motes you want to simulate.
...
You can authenticate to [IoT-LAB](https://www.iot-lab.info/) upfront by running:
